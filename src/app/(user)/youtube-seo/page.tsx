  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-20">
      <PageHeading
        stamp="Engine"
        title="YouTube SEO."
        lede="Judul, deskripsi, dan tag untuk live stream Retro dan AAA, dengan continuity."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="bg-bg border border-line p-6 space-y-6">
            <div className="flex items-center justify-between border-b border-line pb-4">
               <div className="flex items-center gap-2.5">
                  ...
                  <h2 className="text-sm font-bold text-text uppercase tracking-widest">Stream Info</h2>
               </div>
            </div>

            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Nama Game</label>
                <input 
                  type="text" 
                  value={gameName}
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-text-muted flex items-center gap-1.5 uppercase tracking-widest">
                  <TextSelect className="w-3 h-3 text-accent" /> Cerita / Target Momen di Live Ini
                </label>
                <textarea 
                  value={currentProgress}
                  onChange={(e) => setCurrentProgress(e.target.value)}
                  placeholder="Misal: Siap berhadapan dengan Jack Krauser hari ini..."
                  className="w-full bg-bg-elevated border border-line px-4 py-3 text-sm text-text focus:outline-none focus:border-text transition-all h-20 resize-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Vibe Stream</label>
                  <input 
                    type="text" 
                    value={vibe}
                    onChange={(e) => setVibe(e.target.value)}
                    className="w-full bg-bg-elevated border border-line px-4 py-3 text-sm text-text focus:outline-none focus:border-text"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5 pt-4 border-t border-line">
                <div className="flex items-center justify-between mb-1">
                  <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Gunakan Template Gaya</label>
                  <button onClick={() => setIsModalOpen(true)} className="text-[10px] font-bold text-accent hover:text-text flex items-center gap-1 transition-colors">
                ...
                <div className="grid grid-cols-1 gap-2">
                   <button 
                    ...
                    className={`px-4 py-3 text-xs font-medium border text-left flex justify-between items-center transition-colors duration-hover ${selectedTemplate === "none" ? "bg-btn-bg border-text text-btn-fg" : "bg-wash border-line text-text-muted hover:border-text hover:text-text"}`}
                   >
                   ...
                   {templates.map(t => (
                     <div key={t.id} className="group relative">
                        <button 
                          ...
                          className={`w-full px-4 py-3 text-xs font-bold border transition-all text-left flex justify-between items-center ${selectedTemplate === t.id ? "bg-wash border-accent text-accent" : "bg-wash border-line text-text-muted hover:border-text"}`}
                        >
                        ...
                        <button 
                          ...
                          className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-text-faint hover:text-accent opacity-0 group-hover:opacity-100 transition-all"
                        >
        ...
        <div className="lg:col-span-8 space-y-6">
          {!result && !isGenerating ? (
            <div className="bg-bg border border-line p-12 flex flex-col items-center justify-center text-center">
               <div className="w-16 h-16 bg-wash flex items-center justify-center mb-6 border border-line">
                  <MonitorPlay className="w-8 h-8 text-text-faint" />
               </div>
               <h3 className="text-text-muted font-bold uppercase tracking-[0.2em] text-xs">Awaiting Command</h3>
               <p className="text-text-faint text-sm mt-2 max-w-xs">Isi data game dan target ceritanya di samping untuk menghasilkan metadata SEO berkesinambungan.</p>
            </div>
          ) : isGenerating ? (
            <div className="bg-bg border border-line p-12 flex flex-col items-center justify-center text-center">
              <Loader2 className="w-10 h-10 animate-spin text-text-faint mb-6" />
              <h3 className="text-text font-bold uppercase tracking-[0.2em] text-xs">Synthesizing Metadata</h3>
              <p className="text-text-muted text-sm mt-2">Gemini sedang merangkai kata kunci dan menarik memori cerita game dari database...</p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="bg-bg border border-line p-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 text-[10px] font-bold text-accent hover:text-text transition-colors"
                  >
                    {copiedField === 'title' ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                    {copiedField === 'title' ? 'Copied' : 'Copy Judul'}
                  </button>
                </div>
                <p className="text-lg font-bold text-text leading-tight">{result!.title}</p>
              </div>
              ...
              <div className="bg-bg border border-line p-6 space-y-4">
                ...
                <div className="bg-bg-elevated border border-line p-4">
                  <p className="text-sm text-text leading-relaxed whitespace-pre-wrap font-medium">
                    {result!.description}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-bg border border-line p-6 space-y-4">
                ...
                <p className="text-xs text-text-muted font-mono bg-bg-elevated border border-line p-4 leading-loose">
                  {result!.tags}
                </p>
              </div>
            </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-bg/90">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-bg border border-line w-full max-w-lg overflow-hidden">
               <div className="p-6 border-b border-line flex justify-between items-center">
                  <h3 className="text-base font-bold text-text flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-accent" /> Simpan Gaya Bahasa
                  </h3>
               ...
               <form onSubmit={handleSaveTemplate} className="p-6 space-y-5">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Nama Template</label>
                    <input 
                      type="text" required
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Contoh Gaya Judul</label>
                    <textarea 
                      required
                      ...
                      className="w-full bg-bg-elevated border border-line px-4 py-3 text-sm text-text focus:outline-none focus:border-text h-20 resize-none"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Contoh Gaya Deskripsi</label>
                    <textarea 
                      required
                      ...
                      className="w-full bg-bg-elevated border border-line px-4 py-3 text-sm text-text focus:outline-none focus:border-text h-28 resize-none"
                    />
                  </div>
                  <button 
                    type="submit"
