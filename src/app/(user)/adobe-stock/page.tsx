// src/app/(user)/adobe-stock/page.tsx
"use client";
...
export default function AdobeStockGenerator() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      {/* Header Premium Khas Adobe */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-red-600/10 via-background to-background border-l-4 border-l-red-600 border-y border-r border-card-border p-6 rounded-2xl shadow-sm">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-red-600 text-white rounded-lg">
              <Sparkles className="w-5 h-5" />
            </div>
            <h1 className="text-3xl font-bold text-foreground tracking-tight">Adobe Stock Engine</h1>
          </div>
          <p className="text-muted-foreground text-sm max-w-xl">
            Optimasi rasio konversi portofolio Adobe Stock Anda dengan judul dan kata kunci yang dihasilkan secara spesifik oleh Neural Engine.
          </p>
        </div>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input type="number" value={config.titleLengthMin} onChange={e => setConfig({...config, titleLengthMin: Number(e.target.value)})} className="w-full bg-background border border-card-border rounded-lg px-3 py-2.5 text-sm font-medium focus:border-red-500 focus:outline-none transition-colors" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Max Length</label>
                <input type="number" value={config.titleLengthMax} onChange={e => setConfig({...config, titleLengthMax: Number(e.target.value)})} className="w-full bg-background border border-card-border rounded-lg px-3 py-2.5 text-sm font-medium focus:border-red-500 focus:outline-none transition-colors" />
              </div>
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Target Keywords</label>
              <input type="number" value={config.keywordsCount} onChange={e => setConfig({...config, keywordsCount: Number(e.target.value)})} className="w-full bg-background border border-card-border rounded-lg px-3 py-2.5 text-sm font-medium focus:border-red-500 focus:outline-none transition-colors" />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Konteks Desain (Opsional)</label>
              <textarea value={config.conceptContext} onChange={e => setConfig({...config, conceptContext: e.target.value})} placeholder="Sebutkan detail spesifik: halftone, tanpa outline, 3d render..." className="w-full bg-background border border-card-border rounded-lg px-3 py-3 text-sm h-24 resize-none focus:border-red-500 focus:outline-none transition-colors leading-relaxed" />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Negative Terms</label>
              <textarea value={config.negativeKeywords} onChange={e => setConfig({...config, negativeKeywords: e.target.value})} className="w-full bg-background border border-card-border rounded-lg px-3 py-3 text-sm h-20 resize-none focus:border-red-500 focus:outline-none transition-colors leading-relaxed" />
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-card border border-card-border rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              {files.length > 0 && (
                <button onClick={clearAll} disabled={isProcessing} className="text-xs font-medium text-red-500 hover:text-red-400 transition-colors bg-red-500/10 px-3 py-1.5 rounded-lg">
                  Clear Board
                </button>
              )}
            </div>
            ...
            <label 
              ...
              className={`flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-xl cursor-pointer transition-all duration-200 ${
                isDragging ? "border-red-500 bg-red-500/5 scale-[1.01]" : "border-card-border hover:border-red-500/30 hover:bg-accent/30"
              }`}
            >
              <div className={`p-4 rounded-full mb-3 transition-colors ${isDragging ? "bg-red-500/20" : "bg-accent"}`}>
                <UploadCloud className={`w-8 h-8 ${isDragging ? "text-red-500" : "text-muted-foreground"}`} />
              </div>
            ...
            {files.length > 0 && (
              <div className="flex gap-4 mt-6">
                <button 
                  ...
                  className="flex-1 bg-red-600 text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-red-600/20"
                >
          ...
          <div className="space-y-4">
            {files.map((file) => (
              <motion.div key={file.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row gap-5 p-5 border border-card-border rounded-2xl bg-card shadow-sm hover:border-red-500/20 transition-colors">
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[10px] font-bold text-red-500 uppercase tracking-widest">Optimized Title</span>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[10px] font-bold text-red-500 uppercase tracking-widest">SEO Keywords</span>
